...
export class GroupManager {
  ...
  public onConflict: ((elementId: string, fromGroup: string, toGroup: string) => GroupConflictAction | null) | null = null;
  public conflictSuppressed = false;

  public constructor(_overlay: unknown, getElements: () => SvgElement[]) {
    this.getElements = getElements;
  }

  public refreshOverlay(): void {}

  public setOnChange(fn: (() => void) | null): void { this._onChange = fn; }

  public setSelectedGroupIds(ids: string[]): void {
    this.selectedGroupIds.clear();
    for (const id of ids) { if (this.groups.has(id)) this.selectedGroupIds.add(id); }
  ...
  public toggleGroup(id: string): void {
    ...
    else { if (this.groups.has(id)) this.selectedGroupIds.add(id); }
    this.onGroupSelect?.(Array.from(this.selectedGroupIds));
  }

  private notify(): void { this._onChange?.(); }

  public createGroup(name?: string): string {
    const id = 'grp-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6);
    this.groups.set(id, new Group({ id, name: name || `Group-${this.groups.size + 1}`, elementIds: [] }));
    this.notify();
    return id;
  }
  ...
  public deleteGroup(id: string): void {
    ...
    if (!g) return;
    for (const elId of g.elementIds) { const el = this.findElement(elId); if (el) el.groupId = ''; }
  ...
  public addToGroup(groupId: string, elementId: string): void {
    ...
    if (el.groupId && el.groupId !== groupId) {
      if (this.resolveConflict(elementId, el.groupId, groupId) === 'cancel') return;
  ...
  public clearGroup(id: string): void {
    const g = this.groups.get(id);
    if (!g) return;
    for (const elId of g.elementIds) { const el = this.findElement(elId); if (el && el.groupId === id) el.groupId = ''; }
    g.elementIds.clear();
    this.notify();
  }

  public getGroup(id: string): Group | undefined { return this.groups.get(id); }
  public getGroups(): Group[] { return Array.from(this.groups.values()); }
  public getElementIdsInGroup(id: string): string[] { const g = this.groups.get(id); return g ? Array.from(g.elementIds) : []; }

  public getGroupByElement(elementId: string): Group | undefined {
    for (const g of this.groups.values()) { if (g.elementIds.has(elementId)) return g; }
    return undefined;
  }
  ...
  private resolveConflict(elementId: string, fromGroup: string, toGroup: string): GroupConflictAction {
    ...
    if (typeof window !== 'undefined') {
      return window.confirm(`Element "${elementId}" is already in group "${fromGroup}". Move it to "${toGroup}"?`) ? 'move' : 'cancel';
    }
    return 'cancel';
  }

  private findElement(id: string): SvgElement | undefined { return this.getElements().find((e) => e.id === id); }
  public destroy(): void {}
}
