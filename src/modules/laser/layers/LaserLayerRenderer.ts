...
export class LaserLayerRenderer {
  ...
  public init(svg: SVGSVGElement, _camera: Camera): void {
    this.svgRoot = svg;
    this.cameraGroup = svg.querySelector('#cameraGroup') as SVGGElement | null;
  ...
  public build(
    ...
    const filteredLayerInfos = orphanVisible ? layerInfos : layerInfos;
    ...
    for (const el of elements) {
      ...
      useEl.setAttribute(
        'opacity',
        String(Math.round(gResult.opacity * 1000) / 1000),
      );
      ...
      if (gResult.hasCut) {
        ...
        useEl.setAttribute(
          'fill',
          gResult.color !== 'none' ? gResult.color : 'none',
        );
