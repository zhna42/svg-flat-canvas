...
export abstract class AbstractDiff {
    ...
    return new Proxy(obj, {
      set(target, prop: string, value: any, receiver) {
        ...
        if (!self._isApplyingDiff) {
          self._historyDiff[currentPath] = value;
  ...
  private _notifyListeners(path: string, newValue: any): void {
    ...
    if (parts.length > 1) {
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        ...
        if (pl) for (const cb of pl) cb(this[parentPath], parentPath);
      }
    }
  }

  public getHistoryDiff(): DiffData {
    return { ...this._historyDiff };
  }
  public clearHistoryDiff(): void {
    this._historyDiff = {};
  }
  public getBackendDiff(): DiffData {
    return { ...this._backendDiff };
  }
  public clearBackendDiff(): void {
    this._backendDiff = {};
  }
  public isSavedOnBackend(): boolean {
    return Object.keys(this._backendDiff).length === 0;
  }
  public getRenderDiff(): DiffData {
    return { ...this._renderDiff };
  }
  public clearRenderDiff(): void {
    this._renderDiff = {};
  }
  ...
  public setDiff(diff: DiffData, writeToDiffs: boolean = false): void {
    ...
    for (const [path, value] of Object.entries(diff)) {
      ...
      let cur: Record<string, any> = this;
