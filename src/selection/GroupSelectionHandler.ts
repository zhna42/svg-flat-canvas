...
import { createSelectPickCommand, createSelectRectCommand, createSelectLassoCommand } from '@/commands/factories/select-command-factory';
...
export class GroupSelectionHandler {
  ...
  public onMouseDown(wp: { x: number; y: number }, ctrl: boolean, _shift: boolean): boolean {
    const all = this.opts.getElements();
    const gids = hitTestGroupsPoint(wp.x, wp.y, all, this.opts.grid, this.opts.lookupGroup, this.opts.cameraGroup);
  ...
  public onRectEnd(wp: { x: number; y: number }, ctrl: boolean, rectStart: { x: number; y: number }): void {
    const all = this.opts.getElements();
    ...
    if (wasDrag) {
      const gids = hitTestGroupsRect(
        Math.min(wp.x, rectStart.x), Math.min(wp.y, rectStart.y),
        Math.abs(wp.x - rectStart.x), Math.abs(wp.y - rectStart.y),
        all, this.opts.grid, this.opts.lookupGroup, wp.x >= rectStart.x,
      );
      const boxDirection = wp.x >= rectStart.x ? 'left-to-right' : 'right-to-left';
      const cmd = createSelectRectCommand('group', {
        x: Math.min(wp.x, rectStart.x), y: Math.min(wp.y, rectStart.y),
        width: Math.abs(wp.x - rectStart.x), height: Math.abs(wp.y - rectStart.y),
      }, ctrl, boxDirection);
      this.opts.bus.execute(cmd);
      this.opts.onGroupSelect?.(gids);
    }
  ...
  public onLassoEnd(points: { x: number; y: number }[], ctrl: boolean): void {
    if (points.length >= 3) {
      const gids = hitTestGroupsLasso(points, this.opts.getElements(), this.opts.grid, this.opts.lookupGroup);
  ...
  private collectGroupElementIds(groupIds: string[], all: SvgElement[]): SvgElement[] {
    const elementIds = new Set<string>();
    for (const gid of groupIds) {
      const ids = all.filter((e) => this.opts.lookupGroup(e.id) === gid).map((e) => e.id);
