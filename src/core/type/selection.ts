...
export interface SelectionHandlerOptions {
  ...
  getSelectedGroups?: () => import('@/core/shapes/group/Group').Group[];
  getArtboardRect?: () => {
    x: number;
    y: number;
    width: number;
  ...
  events: import('@/core/event-bus/EventBus').EventBus;
}
...
export interface PathNodeActivation {
  element: import('@/core/shapes/elements/PathElement').PathElement;
  cmdIdx: number;
  ptIdx: number;
  startMouseWorld: import('@/core/type').Point;
  startCommands: import('@/core/type').PathCommand[];
  lastMouseWorld: import('@/core/type').Point;
}
